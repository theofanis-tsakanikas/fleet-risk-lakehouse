...
### Added
- **Amazon Managed Grafana via Terraform** (`terraform/04_grafana/` + `modules/aws_grafana/`): a standalone layer that provisions an AMG workspace (IAM Identity Center auth, a customer-managed workspace IAM role, and an SSO ADMIN role association) for operational monitoring of the `pipeline_metrics` fact over the Databricks SQL Warehouse (queried in-app via the Databricks plugin with the read-only BI SPN). Isolated remote state so it can be created for a demo and torn down independently.
